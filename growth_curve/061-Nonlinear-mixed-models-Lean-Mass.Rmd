---
title: '6.1 Nonlinear Mixed Models for the Variable lean mass (''Magra'')'
author: 
...
mod.log <- nls(df0[[y]] ~ ModellogFunc(Edat, beta0, beta1, beta2),
               ...
               data = df0,  control = nls.control(maxiter = 100),
               trace = FALSE)
```
...
nlme.randomAR1asym<- nlme(fitL, random = Asym ~ 1, 
                     control = nlmeControl(maxIter = 100000, msMaxIter = 100),
...
model1 <- paste("mod2", y, "seccio", sep = ".")
mod1 <- update(nlme.randomAR1asym, fixed = list(Asym ~ Seccio, xmid + scal ~ 1),
...
nlme.randomAR1b1g<- nlme(fitL, random = beta1 ~ 1, 
                     control = nlmeControl(maxIter = 100000, msMaxIter = 100),
                     correlation=corAR1(form = ~ 1 | Id),
                     method = "ML")
nlme.randomAR1b2g<- nlme(fitL, random = beta2 ~ 1, 
                     control = nlmeControl(maxIter = 100000, msMaxIter = 100),
                     correlation=corAR1(form = ~ 1 | Id),
                     method = "ML")
...
fx0 <- fixef(nlme.randomAR1b1g)
fx1 <- fixef(nlme.randomAR1b2g)
...
mod1g <- update(nlme.randomAR1b2g, fixed = list(beta0 ~ Seccio, beta1 + beta2 ~ 1),
...
```{r}
mod.lin <- lm(TotalMagra_g~Edat, data= df)
...
mod.lin.log <- lm(TotalMagra_g~log(Edat), data= df)
...
mod.list.combined <- list(
  logistic =get(model0),
  gompert=get(model0g),
  logistic.AR.Asym = get(model1),
  logistic.CAR.Asym = get(model2),
  gompertz.CARb0 = get(model2g),
  lineal = mod.lin
)
...
newdat <- within(newdat, {
  Predicted <- growthmodels::gompertz(Edat, 
...
newdat <- rbind(newdat, data.frame(Seccio = "FUTBOL SALA", 
                                    Edat = seq(min(df$Edat), max(df$Edat), length.out = 100),
                                   Predicted = growthmodels::gompertz(seq(min(df$Edat), max(df$Edat), length.out = 100),
...
p <- ggplot(df, aes(x = Edat, y = get(y), colour = Seccio)) +
  ...
  geom_line(aes(y = predict(get(model2g)), group = Id), alpha = 0.3) + 
  labs(y = y)+
